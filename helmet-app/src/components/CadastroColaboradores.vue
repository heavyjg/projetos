  ...
  <v-container grid-list-md text-xs-center>
    <v-layout row wrap>
      <v-flex xs8 offset-xs2>
        ...
        <v-text-field label="Nome do colaborador" v-model="nome" :rules="nameRules" required></v-text-field>
        <v-text-field label="Cargo do colaborador" v-model="cargo" :rules="cargoRules" required></v-text-field>
        <v-text-field label="CPF do colaborador" v-model="cpf" :mask="cpfMask" :rules="cpfRules" required></v-text-field>
        <v-text-field name="input-7-1" label="Função do colaborador" v-model="description" multi-line></v-text-field>
